...
namespace agora { namespace tools {
    ...
    struct DynamicKey{
        ...
        bool fromString(const std::string& dynamicKeyString)
        {
            ...
            this->signature = dynamicKeyString.substr(L::SIGNATURE_OFFSET, SIGNATURE_LENGTH);
            this->staticKey = dynamicKeyString.substr(L::STATIC_KEY_OFFSET, STATIC_KEY_LENGTH);
            try {
                this->unixTs =std::stoul(dynamicKeyString.substr(L::UNIX_TS_OFFSET, UNIX_TS_LENGTH),nullptr, 10);
                this->randomInt =std::stoul(dynamicKeyString.substr(L::RANDOM_INT_OFFSET, RANDOM_INT_LENGTH),nullptr, 16);
